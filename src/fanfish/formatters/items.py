...
recipes: list[etree._Element] = [
    recipe for root in data for recipe in root.findall("recipe", None)
]
quests: list[etree._Element] = [
    quest for root in data for quest in root.findall("quest", None)
]
enemies: list[etree._Element] = [
    enemy for root in data for enemy in root.findall("enemy", None)
]
...
for enemy in enemies:
    loot_items: list[etree._Element] = [
        *enemy.findall("./lootSet/loot", None),
        *enemy.findall("./loot", None),
    ]
...
for quest in quests:
    ...
    for item in required_items:
        used_for_quests.setdefault(item.get("id", None), []).append(
            quest.get("id", None)
        )
...
for recipe in recipes:
    ...
    for item in required_items:
        used_for_recipes.setdefault(item.get("id", None), []).append(
            recipe.get("id", None)
        )
...
for item in items:
    ...
    if familiar is not None:
        for food in item.findall("food", None):
            familiars_eat.setdefault(food.get("id", None), []).append(
                item.get("id", None)
            )
