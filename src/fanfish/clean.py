...
from fanfish.xmlparser import XmlNode, parse
...
replacements = [
    (re.compile(pattern), replacement) for pattern, replacement in replacements.items()
]
...
for file in data_folder.rglob("*.xml"):
    ...
    for child in (node, *node.get_children(recursive=True)):
        ...
        child.attributes = {
            key: escape(value) for key, value in child.attributes.items()
        }
